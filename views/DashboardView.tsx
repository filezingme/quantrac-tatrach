  ...
  return (
    <div className="space-y-6 animate-fade-in relative pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
           <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Dữ liệu quan trắc</h2>
           <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2 mt-1">
             <Calendar size={14}/>
             Cập nhật: {lastUpdated}
             <button onClick={refreshData} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 p-1 hover:bg-blue-50 dark:hover:bg-slate-700 rounded-full transition-colors"><RefreshCw size={14}/></button>
           </p>
        </div>
        <div className="flex gap-2">
            <div className="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg text-xs font-bold uppercase tracking-wide border border-green-200 dark:border-green-800 shadow-sm flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div> Máy chủ: Online
            </div>
            <div className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg text-xs font-bold uppercase tracking-wide border border-blue-200 dark:border-blue-800 shadow-sm">
                Sensor: Tốt
            </div>
        </div>
      </div>
      ...
      {selectedMetric && (
        <div className={`fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200 ${isFullscreen ? 'p-0' : 'p-4'}`}>
          <div className={`bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 ${isFullscreen ? 'w-full h-full rounded-none' : 'w-full max-w-6xl h-[85vh]'}`}>
