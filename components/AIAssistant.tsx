...
export const AIAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (error.message?.includes('API key not found')) {
          errorMsg = 'Lỗi: Không tìm thấy API Key hợp lệ.';
      }
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'model', type: 'error', content: errorMsg }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const containerClasses = isFullScreen 
    ? 'inset-0 z-[4000] rounded-none' 
    : isExpanded 
      ? 'inset-4 md:inset-20 z-[3000] rounded-2xl' 
